...
module Administrating
  class RegisterChipTest < Infra::DomainTestHelper
    test 'a chip is registered' do
      ...
      published = act(stream, RegisterChip.new(uuid: uuid, number: 12345, registry: "ThirdPartyRegistry"))
      assert_changes(published, [ChipRegistered.new(data: { animal_uuid: uuid, number: 12345, registry: "ThirdPartyRegistry"})])
    end
    ...
    test 'a chip is added to an animal that is not registered' do
      ...
      assert_raises(Animal::NotRegistered) do
        act(stream, RegisterChip.new(uuid: uuid, number: 12345, registry: "ThirdPartyRegistry"))
      end
    end
  end
end
